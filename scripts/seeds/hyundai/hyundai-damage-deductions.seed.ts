/**
 * Hyundai Damage Deduction Seed Script
 * 
 * Source: Hyundai & Kia in Nigeria Comprehensive Price & Valuation Guide (Feb 2026)
 * Extracted from: scripts/import-hyundai-kia-damage-deductions.ts (Hyundai portion)
 * Note: Hyundai and Kia share platform engineering under Hyundai Motor Group
 * 
 * Usage:
 * - Test with --dry-run: tsx scripts/seeds/hyundai/hyundai-damage-deductions.seed.ts --dry-run
 * - Run: tsx scripts/seeds/hyundai/hyundai-damage-deductions.seed.ts
 * - Force re-run: tsx scripts/seeds/hyundai/hyundai-damage-deductions.seed.ts --force
 * 
 * Features:
...
const SCRIPT_NAME = 'hyundai-damage-deductions';
const BATCH_SIZE = 50;
...
/**
 * Hyundai damage deduction data from official Hyundai & Kia guide (Feb 2026)
 * These deductions are shared between Hyundai and Kia due to platform engineering
 */
const hyundaiRawData = [
  // Front Bumper
  {
    make: 'Hyundai',
...
async function executeSeed() {
  ...
  console.log(`📊 Total raw records: ${hyundaiRawData.length}\n`);
  ...
  try {
    ...
    const records = transformToDbRecords(hyundaiRawData);
