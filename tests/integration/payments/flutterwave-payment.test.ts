...
/**
 * Integration Test: Flutterwave Payment Flow
 * 
...
describe('Flutterwave Payment Integration', () => {
  let testUser: any;
  let testVendor: any;
  let testCase: any;
  let testAuction: any;
  ...
  beforeAll(async () => {
    ...
    [testUser] = await db
      .insert(users)
      .values({
        email: `test-flw-payment-${Date.now()}@example.com`,
        phone: `+234${Math.floor(Math.random() * 10000000000)}`,
        passwordHash: 'test-hash',
        role: 'vendor',
        status: 'verified_tier_1',
        fullName: 'Test Flutterwave Vendor',
    ...
    [testCase] = await db
      .insert(salvageCases)
      .values({
        claimReference: `CLM-FLW-${Date.now()}`,
        assetType: 'vehicle',
        assetDetails: { make: 'Honda', model: 'Accord', year: 2019 },
        marketValue: '4500000',
        estimatedSalvageValue: '1350000',
        reservePrice: '945000',
  ...
  describe('Payment Initiation', () => {
    it('should initiate payment and generate payment link', async () => {
      ...
      expect(result.reference).toMatch(/^FLW_/);
      expect(result.amount).toBe(1350000);
      ...
      expect(payment?.paymentMethod).toBe('flutterwave');
  ...
  describe('Webhook Processing', () => {
    ...
    beforeEach(async () => {
      ...
      webhookPayload = {
        ...
        data: {
          ...
          currency: 'NGN',
          charged_amount: parseFloat(paymentRecord.amount),
          status: 'successful',
          payment_type: 'card',
      ...
      const webhookSecret = process.env.FLUTTERWAVE_WEBHOOK_SECRET || 'test-webhook-secret';
    ...
    it('should process webhook and auto-verify payment', async () => {
      ...
      await processFlutterwaveWebhook(webhookPayload, webhookSignature, rawPayload);
    ...
    it('should reject webhook with invalid signature', async () => {
      ...
      await expect(
        processFlutterwaveWebhook(webhookPayload, invalidSignature, rawPayload)
    ...
    it('should reject webhook with amount mismatch', async () => {
      const tamperedPayload = {
        ...
        data: {
          ...webhookPayload.data,
          amount: webhookPayload.data.amount + 100000, // Add 100,000 naira
        },
      };
      ...
      await expect(
        processFlutterwaveWebhook(tamperedPayload, tamperedSignature, rawPayload)
    ...
    it('should reject webhook with invalid currency', async () => {
      ...
      const rawPayload = JSON.stringify(invalidCurrencyPayload);
      const webhookSecret = process.env.FLUTTERWAVE_WEBHOOK_SECRET || 'test-webhook-secret';
      const invalidCurrencySignature = crypto
        .createHmac('sha256', webhookSecret)
        .update(rawPayload)
        .digest('hex');

      await expect(
        processFlutterwaveWebhook(invalidCurrencyPayload, invalidCurrencySignature, rawPayload)
    ...
    it('should ignore non-completed events', async () => {
      ...
      await processFlutterwaveWebhook(failedPayload, failedSignature, rawPayload);
