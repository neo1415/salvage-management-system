  ...
  for (const provider of ['gemini', 'claude'] as const) {
    ...
    try {
      ...
      if (provider === 'gemini') {
        ...
        const response = await client.messages.create({model:process.env.CLAUDE_PRICE_ADJUDICATION_MODEL || process.env.CLAUDE_MODEL || 'claude-sonnet-4-6',max_tokens:8192,temperature:0.1,messages:[{role:'user',content:prompt}]});
      ...
      console.warn('[Repair estimation] Provider failed; trying the remaining model', {provider, errorType: error instanceof Error ? error.name : 'unknown'});
    }
  }
  return estimates;
}
