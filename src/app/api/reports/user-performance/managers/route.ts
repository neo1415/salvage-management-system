...
import { ManagerMetricsService } from '@/features/reports/user-performance/services';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!ReportService.hasPermission(session.user.role, 'manager-metrics')) {
    ...
    const result = await ReportService.generateReport(
      { type: 'manager-metrics', filters, includeCharts: true },
      session.user.id,
      session.user.role,
      async (filters) => await ManagerMetricsService.generateReport(filters),
