...
import { Camera, CheckCircle2, ClipboardCheck, Loader2, PackageCheck, Phone, UploadCloud } from 'lucide-react';
        ...
        {pickups.length === 0 ? (
          ...
          <div className="grid gap-5">
            {pickups.map((pickup) => {
              ...
              const pickupTelephoneHref = getTelephoneHref(pickup.pickupContact.phone);
              return (
                <section key={pickup.auctionId} className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                  ...
                  {(pickupTelephoneHref || pickup.pickupContact.email) && (
                    <div className="mt-4 flex flex-col gap-3 border border-blue-200 bg-blue-50 p-4 text-blue-950 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <p className="text-sm font-semibold">Contact the pickup team</p>
                        <p className="text-sm text-blue-800">{pickup.pickupContact.name}: {pickupTelephoneHref ? pickup.pickupContact.phone : pickup.pickupContact.email}</p>
                      </div>
                      {pickupTelephoneHref ? (
                        <a
                          href={pickupTelephoneHref}
                          className="inline-flex items-center justify-center gap-2 rounded-lg bg-[var(--brand-primary)] px-4 py-2 text-sm font-semibold text-white"
                        >
                          <Phone className="h-4 w-4" aria-hidden="true" />
                          Call pickup team
                        </a>
                      ) : (
                        <a href={`mailto:${pickup.pickupContact.email}`} className="inline-flex items-center justify-center rounded-lg bg-[var(--brand-primary)] px-4 py-2 text-sm font-semibold text-white">
                          Email pickup team
                        </a>
                      )}
                    </div>
