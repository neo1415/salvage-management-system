...
import type { FinanceMetricsReport } from '@/features/reports/user-performance/services';

export default function FinancePerformancePage() {
  const router = useAppRouter();
  const { loading, isRefreshing, startFetch, endFetch, markHasData } =
    useReportFetchState();
  const [reportData, setReportData] = useState<FinanceMetricsReport | null>(null);
  ...
  const fetchReport = useCallback(async (force = false) => {
    ...
    try {
      ...
      if (result.status === 'success') {
        setReportData(result.data as FinanceMetricsReport);
        markHasData();
      }
    } catch (error) {
